...
type Workflows interface {
	// Design は指定されたキャラクターIDのキャラクターを生成します。aspectRatio・layoutKind・
	// override は go-manga-kit/runner.MangaDesignRunner.Run のドキュメントを参照してください。
	Design(ctx context.Context, charIDs []string, seed int64, outputDir, aspectRatio, layoutKind string, override ports.DesignOverride) (string, int64, error)
